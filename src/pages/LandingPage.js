...
const LandingPage = () => {
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117', color: '#e2e8f0', fontFamily: 'Inter, sans-serif', overflowX: 'hidden' }}>
      ...
      {/* Navbar */}
      <nav style={{ padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'fixed', top: 0, width: '100%', zIndex: 100, background: 'rgba(13,17,23,0.9)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <Link to="/login" style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', color: '#e2e8f0', textDecoration: 'none', fontSize: 14, fontWeight: 500, transition: 'all 0.2s' }}>Sign In</Link>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '120px 40px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 800, textAlign: 'center', position: 'relative' }}>
          ...
          <p className="fade-up delay-3" style={{ fontSize: 18, color: '#8892a4', lineHeight: 1.7, marginBottom: 40, maxWidth: 560, margin: '0 auto 40px' }}>
      ...
      <section style={{ padding: '80px 40px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          <h2 style={{ fontFamily: 'Space Grotesk', fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 700, marginBottom: 16 }}>
            Everything you need to manage USDT
          </h2>
          <p style={{ color: '#8892a4', fontSize: 17, maxWidth: 500, margin: '0 auto' }}>
            A complete financial platform designed for security and ease of use.
          </p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
          {[
            { icon: <FiShield size={24} />, color: '#00d4a3', title: 'Bank-Level Security', desc: 'JWT authentication, PIN protection, rate limiting, and enterprise-grade encryption on every transaction.' },
            { icon: <FiZap size={24} />, color: '#7c6ef7', title: 'Instant Transfers', desc: 'Send USDT to any registered NexVault user instantly with just their email address.' },
            { icon: <FiGlobe size={24} />, color: '#ffa502', title: 'Global Deposits', desc: 'Deposit USDT from anywhere in the world with QR code and wallet address support.' },
            { icon: <FiLock size={24} />, color: '#ff4757', title: 'Withdrawal PIN', desc: 'Every withdrawal requires a secure 4-digit PIN to prevent unauthorized transactions.' },
            { icon: <FiTrendingUp size={24} />, color: '#00d4a3', title: 'Transaction History', desc: 'Complete transaction history with timestamps, IDs, amounts, and status tracking.' },
            { icon: <FiCheckCircle size={24} />, color: '#7c6ef7', title: 'Admin Verified', desc: 'All deposits and withdrawals are manually verified by admins for maximum security.' },
          ].map(f => (
            <div key={f.title} className="feature-card" style={{ padding: 28, borderRadius: 16, background: '#161b27', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: `rgba(${f.color === '#00d4a3' ? '0,212,163' : f.color === '#7c6ef7' ? '124,110,247' : f.color === '#ffa502' ? '255,165,2' : '255,71,87'},0.12)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: f.color, marginBottom: 16 }}>
                {f.icon}
              </div>
      ...
      <section style={{ padding: '80px 40px', textAlign: 'center' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', padding: 48, borderRadius: 24, background: 'linear-gradient(135deg, rgba(0,212,163,0.08), rgba(124,110,247,0.08))', border: '1px solid rgba(0,212,163,0.2)' }}>
          <h2 style={{ fontFamily: 'Space Grotesk', fontSize: 36, fontWeight: 700, marginBottom: 16 }}>
            Start managing your USDT today
          </h2>
          <p style={{ color: '#8892a4', marginBottom: 32, fontSize: 16 }}>
            Join thousands of users who trust NexVault for secure USDT management.
          </p>
